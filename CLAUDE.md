...
- **Supervisor** (`supervisor.py`): 接收用户输入，LLM 驱动子任务拆分（`plan_task()`），管理 Ticket 生命周期，状态机（`SupervisorState` 枚举：idle→planning→dispatching→waiting_worker→reviewing→complete/failed），REPL 交互。Memory 在此级别共享。REPL 支持 `/tickets`、`/ticket <id>`、`/revise <id> <描述>`、`/continue [id]`、`/diff`、`/verify`、`/checkpoint` 和 `/rollback`。
