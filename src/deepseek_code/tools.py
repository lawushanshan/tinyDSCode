...
class Tools:
        ...
        while pos < len(patch_lines):
            ...
            if line.startswith("@@"):
                ...
                if old_start < line_index:
                    raise ValueError(f"diff hunk 顺序错误或重复: {header}")
    ...
    _BINARY_EXTENSIONS = {
        ".pyc", ".pyo", ".exe", ".dll", ".so", ".dylib", ".bin", ".png",
        ".jpg", ".jpeg", ".gif", ".zip", ".tar", ".gz", ".whl", ".egg",
    }
...
def create_default_registry() -> ToolRegistry:
    ...
    registry.register(ToolDef(
        ...
        parameters=[
            ToolParam(name="pattern", type="string", description="文件名模式，例如 **/*.py"),
            ToolParam(name="path", type="string", description="搜索根目录，默认当前目录", required=False),
            ToolParam(name="exclude_patterns", type="string", description="排除模式（逗号分隔），例如 node_modules,.git", required=False),
        ],
        handler=lambda pattern, path=".", exclude_patterns=None: Tools.search_files(
            pattern,
            path,
            exclude_patterns.split(",") if exclude_patterns else None,
        ),
    ))
    registry.register(ToolDef(
        name="search_content",
        ...
        parameters=[
            ...
            ToolParam(name="path", type="string", description="搜索根目录，默认当前目录", required=False),
            ToolParam(name="include", type="string", description="只搜索匹配的文件名，例如 *.py", required=False),
            ToolParam(name="exclude", type="string", description="排除匹配的文件名，例如 *.log", required=False),
            ToolParam(name="context_lines", type="integer", description="上下文行数，默认 0", required=False),
        ],
        handler=lambda pattern, path=".", include=None, exclude=None, context_lines=0: Tools.search_content(
            pattern,
            path,
            include,
            exclude,
